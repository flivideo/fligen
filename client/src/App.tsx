    ...
    <div className="flex h-screen w-screen bg-slate-900 text-white">
      ...
      <aside className="w-[200px] shrink-0 bg-slate-800 border-r border-slate-700 flex flex-col">
        <nav className="flex-1 py-2 overflow-y-auto">
          {DAYS.map((d) => (
            ...
            >
              {showDayIcons && <span className="text-lg shrink-0">{d.icon}</span>}
              <span className="flex-1 truncate text-sm">
                Day {d.day} - {d.shortName}
              </span>
              <span
                className={
                  d.status === 'complete'
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <footer className="h-8 shrink-0 bg-slate-800 border-t border-slate-700 flex items-center px-4 text-xs">
          <span
            className={`w-2 h-2 rounded-full mr-2 ${socketConnected ? 'bg-green-400' : 'bg-red-400'}`}
          />
      ...
      {showApiExplorer && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-[95vw] h-[90vh] bg-slate-900 rounded-lg border border-slate-700 flex flex-col">
            {/* Header */}
            <div className="flex items-center justify-between p-4 border-b border-slate-700">
              <div>
                <h2 className="text-lg font-semibold text-white">API Explorer</h2>
                <p className="text-sm text-slate-400">Interactive API documentation and testing</p>
              </div>
              <button
